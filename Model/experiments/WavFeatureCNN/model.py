...
class AutoencoderModel:
    def __init__(self, config: Dict):
        ...
        Args:
            config: 配置字典，包含模型參數
        """
        self.time_steps = config.get('time_steps', 512)  # 固定時間步長
        self.feature_dim = config.get('feature_dim', 20000)  # 固定特徵維度
    ...
    def build(self):
        ...
        # 第一個卷積塊
        x = Conv1D(64, 7, padding='same', name='conv1')(inputs)
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        ...
        Args:
            x: 輸入數據，形狀為 (batch_size, 512, 20000)
